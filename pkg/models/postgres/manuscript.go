...
func (m *ManuscriptModel) AutoMigrate() {
	m.DB.AutoMigrate( &models.Manuscript{} )

	var manuscripts []models.Manuscript
	m.DB.Find(&manuscripts)
	if len(manuscripts) == 0 {

		m.DB.Create(&models.Manuscript{
			Authors:  "Dyer RJ",
			Year:     2007,
...
func (m *ManuscriptModel) Insert( Authors string, Year int, Title string, Journal string, Volume int, Pages string, Doi string, Abstract string ) (int, error) {
	return 0, nil 
}

func (m *ManuscriptModel) Get(id int) (*models.Manuscript, error ) {
	return nil, nil 
}

func (m *ManuscriptModel) Latest() ([]*models.Manuscript, error) {
	return nil, nil 
}
